...
<div id="modal-hasil-terapi" style="display: none">
  <div class="card-box">
    <table class="table table-keterangan table-striped">
      ...
      <tr>
        <th>Pertemuan ke</th>
        <td>:</td>
        <td><span id="pertemuan_ke"></span></td>
      </tr>
    </table>
  </div>
  <div class="card-box">
...
<script>
  ...
  $("#modal-hasil-terapi").iziModal({
    title: 'Data Hasil Terapi',
    subtitle: '',
    headerColor: '#676a6c',
    //   background: null,
    //   theme: '',  
  ...
  {
   axios.get('{{ url('hasil_terapi/') . '/' }}'+id, {
   })
   .then(res => {
    response = res.data;
    $('#modal-hasil-terapi').iziModal('open');
    set_modal_data(response);
    $("#modal-hasil-terapi .iziModal-wrap").scrollTop(0);            
 ...
 var set_modal_data = function(data)
 {
  ...
  $("#tanggal").text(data.tanggal_manusia);
  $("#pertemuan_ke").text(data.pertemuan_ke);
